...
Detects: Information disclosure, security headers, cookies, mixed content,
         ...
         OAuth misconfigurations, and API security issues.
"""
...
<body>
    <div class="container">
        ...
        <div class="warning">
            <strong>⚠️ Security Alert:</strong> <code>{self.url}</code> is vulnerable to clickjacking.
        </div>
        <h2>Vulnerability Details</h2>
        <ul>
            <li><strong>Target URL:</strong> <code>{self.url}</code></li>
...
# -----------------------------------------------------------------------------
# Enhanced PCAP Processor with Streaming
# -----------------------------------------------------------------------------

class EnhancedPCAPProcessor:
    """Enhanced PCAP processor with streaming, TCP reassembly, and HTTP/2 support"""
    ...
    def process_pcap(self, filepath: str) -> List[Tuple[str, requests.Response]]:
...
def print_scan_summary(total_targets: int, findings: List[SecurityFinding], 
                       error_count: int, duration: float, scan_mode: str):
    """Print scan summary"""
...
def print_all_findings_detailed(findings: List[SecurityFinding]):
    """Print all findings with complete details"""
    if not findings:
        return
    
    print(f"\n{Colors.BOLD}{Colors.CYAN}🔍 COMPLETE FINDINGS LIST ({len(findings)} findings){Colors.END}")
    print(f"{Colors.CYAN}{'='*80}{Colors.END}")
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="Detects: GraphQL, JWT, HTTP Smuggling, Cache Poisoning, SSRF, OAuth, and more"
    )
    ...
    try:
        if file_ext in ['.har']:
            ...
            responses_with_urls = processor.process_pcap(args.file)
            scan_mode = "Passive (PCAP Analysis with Streaming)"
    ...
    unique_findings.sort(key=lambda x: (severity_order.get(x.severity, 5), x.url))
    
    duration = time.time() - start_time
    total_targets = len(responses_with_urls) if responses_with_urls else len(targets)
    print_scan_summary(total_targets, unique_findings, len(error_list), duration, scan_mode)
    ...
    print(f"{Icons.INFO} {Colors.BOLD}Total findings displayed:{Colors.END} {Colors.YELLOW}{len(unique_findings)}{Colors.END}")
    print(f"{Colors.CYAN}{'='*80}{Colors.END}\n")
