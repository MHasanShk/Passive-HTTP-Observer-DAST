...
class CookieParser:
        ...
        This provides consistent and reliable attribute extraction across all Python versions.
        """
        
        @staticmethod
        def parse_set_cookie_header(header_value):
            """
            Parse a single Set-Cookie header value and return a dictionary of cookie attributes.
            
            Args:
                header_value (str): The raw Set-Cookie header value
                
            Returns:
                dict: Dictionary containing cookie name, value, and all attributes
            """
            try:
                # SimpleCookie handles the parsing robustly
                cookie = SimpleCookie()
                cookie.load(header_value)
                
                # Extract the first cookie (SimpleCookie can handle multiple, but typically one per header)
                for cookie_name, morsel in cookie.items():
                    return {
                        ...
                        'version': morsel.get('version', None),
                        'comment': morsel.get('comment', None),
                        'raw_value': header_value
                    }
            except Exception as e:
                # Fallback for malformed cookies
                return {
                    'name': 'unknown',
                    'value': header_value,
                    'secure': False,
        ...
        def parse_all_cookies(response):
            """
            Extract and parse all Set-Cookie headers from a response.
            
            Args:
                response: requests.Response object
                
            Returns:
                list: List of cookie dictionaries with all attributes parsed
            """
            cookies = []
            
            # Get all Set-Cookie headers (there could be multiple)
            set_cookie_headers = response.headers.get_list('Set-Cookie') if hasattr(response.headers, 'get_list') else []
            
            # If get_list isn't available, get the raw headers
            if not set_cookie_headers:
                # Get raw headers from response
                raw_headers = response.raw.headers if hasattr(response, 'raw') else []
                set_cookie_headers = [v for k, v in response.headers.items() if k.lower() == 'set-cookie']
            
            # Parse each Set-Cookie header
            for header in set_cookie_headers:
                cookie_info = CookieParser.parse_set_cookie_header(header)
                if cookie_info:
                    cookies.append(cookie_info)
            
            # Also try to get from response.cookies (fallback for compatibility)
            for cookie in response.cookies:
                # Check if we already have this cookie from header parsing
                existing = next((c for c in cookies if c['name'] == cookie.name), None)
                if not existing:
                    # Parse attributes from the cookie object
                    samesite = None
        ...
        @staticmethod
        def is_httponly(cookie_info):
            """
            Check if cookie has HttpOnly flag.
            
            Args:
                cookie_info (dict): Cookie dictionary from parse_set_cookie_header
                
            Returns:
                bool: True if HttpOnly flag is set
            """
            return cookie_info.get('httponly', False)
        
        @staticmethod
        def is_secure(cookie_info):
            """
            ...
            Returns:
                bool: True if Secure flag is set
            """
            return cookie_info.get('secure', False)
        
        @staticmethod
        def get_samesite(cookie_info):
            """
            ...
            Returns:
                str or None: SameSite value (Strict, Lax, None) or None if not set
            """
            samesite = cookie_info.get('samesite', None)
            if samesite:
                # Normalize case
                return samesite.lower().capitalize()
            return None
        
        @staticmethod
        def get_expiration_info(cookie_info):
            """
            Get cookie expiration information.
            
            Args:
                cookie_info (dict): Cookie dictionary from parse_set_cookie_header
                
            Returns:
                dict: Expiration information including type and time
            """
...
class PassiveHttpObserver:
        ...
        def analyze_target(self, url, check_backups=False):
                ...
                try:
                        ...
                        # 3. ENHANCED Cookie Security Analysis (using new cookie parser)
        ...
        # -------------------------------------------------------------------------
        # ANTI-CLICKJACKING HEADER ANALYSIS MODULE
        # -------------------------------------------------------------------------
        ...
        # -------------------------------------------------------------------------
        # ENHANCED SESSION COOKIE SECURITY ANALYSIS MODULE (REWRITTEN)
        # -------------------------------------------------------------------------
        
        def _check_enhanced_cookie_security(self, url, response):
                """
                Comprehensive session cookie security analysis using robust cookie parser.
                This version uses http.cookies.SimpleCookie for reliable attribute extraction.
                """
                findings = []
                ...
                # Parse all cookies using the robust cookie parser
                ...
                for cookie_info in parsed_cookies:
                        ...
                        if expiration_info.get('type') == 'max-age':
                                max_age_seconds = expiration_info.get('seconds', 0)
                                max_age_days = max_age_seconds / 86400
                                
                                if max_age_days > 90:
                                        issues.append(f"Cookie max-age is {max_age_days:.0f} days (excessively long)")
                                        recommendations.append("Reduce max-age to 90 days or less")
                                        if is_session_cookie and severity != "Critical":
                                                severity = "Medium"
                                elif max_age_seconds < 3600 and max_age_seconds > 0 and is_session_cookie:
                                        issues.append(f"Session cookie expires in {int(max_age_seconds/60)} minutes (very short)")
                                        recommendations.append("Extend session lifetime to reduce user disruption")
                                        severity = "Low"
                        
                        elif expiration_info.get('type') == 'expires':
                                # We don't parse exact dates here, but note that it's not a session cookie
                                if not is_session_cookie:
                                        pass  # Non-session cookies with expiration are generally fine
                        
                        elif expiration_info.get('type') == 'session':
                                # Session cookies are generally OK but need proper flags
                                if not is_session_cookie:
                                        issues.append("Non-session cookie has no expiration (persists until browser close)")
                                        recommendations.append("Set an appropriate Max-Age or Expires attribute for non-session cookies")
                                        severity = "Low"
                        
                        # 7. Session Fixation Vulnerability Check
                        if is_session_cookie and self._check_session_fixation_vulnerability_cookie(cookie_name, cookie_value):
                                issues.append("Session cookie has predictable pattern - vulnerable to session fixation")
                                recommendations.append("Generate cryptographically random session IDs with sufficient entropy (minimum 128 bits)")
                        ...
                        findings.extend(self._check_cookie_prefixes_enhanced(url, cookie_info, response))
                ...
                if auth_cookies:
                        ...
                        for cookie_info in parsed_cookies:
                                if any(kw in cookie_info['name'].lower() for kw in ['session', 'auth', 'token', 'jwt', 'sid']):
                                        if self._is_cookie_properly_configured(cookie_info):
                ...
                # If no findings but cookies exist
                if not findings and parsed_cookies:
                        findings.append(SecurityFinding(
                                url,
                                "Cookie Security Analysis Complete",
                                f"Analyzed {len(parsed_cookies)} cookie(s), all properly configured",
        ...
        def _check_session_fixation_vulnerability_cookie(self, cookie_name, cookie_value):
                """
                Check for potential session fixation vulnerabilities using enhanced detection
                """
                # Look for common session fixation indicators
                fixation_indicators = [
                    len(cookie_value) < 16,  # Too short (less than 16 chars/128 bits)
                    re.match(r'^[0-9]+$', cookie_value) is not None,  # Only numbers
                    re.match(r'^[a-z]+$', cookie_value) is not None,  # Only lowercase letters
                    re.match(r'^[A-Z]+$', cookie_value) is not None,  # Only uppercase letters
                    re.match(r'^[0-9A-Fa-f]+$', cookie_value) is not None and len(cookie_value) < 32,  # Hex but too short
                    'PHPSESSID' in cookie_name,  # PHP default (often predictable)
                    'jsessionid' in cookie_name.lower(),  # Java default
                    'aspsessionid' in cookie_name.lower(),  # ASP default
                    'ci_session' in cookie_name.lower(),  # CodeIgniter default
                    'laravel_session' in cookie_name.lower(),  # Laravel default
                ]
                
                # Check for timestamp patterns in cookie value (often indicate predictability)
                if re.search(r'\d{10,}', cookie_value):  # Unix timestamp
                        fixation_indicators.append(True)
                
                # Check for sequential patterns
                if re.search(r'(123|abc|000|111|222|333)', cookie_value.lower()):
                        fixation_indicators.append(True)
                
                # If any indicators match, potential vulnerability
                return any(fixation_indicators)
        
        def _is_cookie_properly_configured(self, cookie_info):
                """
                ...
                return is_secure and is_httponly and samesite and samesite.lower() in ['lax', 'strict']
        
        def _check_cookie_prefixes_enhanced(self, url, cookie_info, response):
                """
                ...
                # Check for __Host- prefix
                    ...
                    if cookie_info.get('domain'):
                        issues.append(f"Has Domain attribute '{cookie_info['domain']}' (__Host- prefix requires no Domain attribute)")
                    
                    # Path must be exactly "/"
                    if cookie_info.get('path') != '/':
                        issues.append(f"Path is '{cookie_info.get('path')}' but must be exactly '/' for __Host- prefix")
                ...
                # Advisory for session cookies that could benefit from prefixes
                    ...
                    if not cookie_name.startswith('__Secure-') and not cookie_name.startswith('__Host-'):
                        if self.cookie_parser.is_secure(cookie_info):
                            findings.append(SecurityFinding(
                                url,
...
{Colors.CYAN}{'='*80}{Colors.END}
{Colors.DIM}    Version: 4 | Author: Security Testing Framework | Mode: Passive Analysis{Colors.END}
{Colors.CYAN}{'='*80}{Colors.END}
        """
        print(banner)
